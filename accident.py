...
def build_model():
    ...
    lstm_cell = tf.nn.rnn_cell.LSTMCell(n_hidden, n_hidden,
    ...
    zeros_object = tf.to_float(
        tf.not_equal(tf.reduce_sum(tf.transpose(x[:, :, 1:n_detection, :], [1, 2, 0, 3]), 3), 0))  # frame x n x b
    # Start creat graph
    for i in range(n_frames):
        ...
        image_part = tf.matmul(n_object, tf.tile(tf.expand_dims(weights['att_ua'], 0), [n_detection - 1, 1, 1])) + \
                     biases['att_ba']  # n x b x h
